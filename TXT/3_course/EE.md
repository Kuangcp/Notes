...
get：
	...
	一方：配置文件中set标签 加上lazy="false"（两条SQL），再添加 fetch="join"后就只有一条SQL语句，但是一方是不能分页查询的
load：
	在你确定OID是一定有的时候使用load提高效率

注意：懒加载如果Session关闭了或者是对象游离态。就会有懒加载初始化的异常
*****************************************************************************
##7.【Mybatis】
###xml文件配置：
